...
export default function ModalSelectLeitura(props: IProps) {
    ...
    function UpdateSelectedFields(field: string, value: string, keyName: string, keyValue: number) {
        setSelectedField(prevState => { return { ...prevState, [field]: value, [keyName]: keyValue } })
    }
    async function ClickTestamento(valueIdTestamento: number) {//ao selecionar um testamento, irá solicitar os livros enviando  o id do testamento
        let response: IresultLivros[] = await FetchAPILivros(valueIdTestamento)
        setLivros(response)
        setCapitulo([])
    ...
    async function ClickLivro(livroIDSelect: number) {// ao selecionar livro, irá solicitar os capitulos do livro selecionado
        if (livroIDSelect != 0) { //Se o livroIDSelect for  diferente de 0, é pq o livro  está com a opção diferente de  => selecione
                ...
                <div className={styles.modalmain}>
                    <div className={styles.modalopcoes}>
                        <div className={styles.modalopcoesversao} >
                            ...
                            <select name="versao" value={selectedField.versao} onChange={(evt) => { UpdateSelectedFields("versao", evt.target.value, "versao_id", evt.target.options.selectedIndex) }}>
                        ...
                        <div className={styles.modalopcoestestamento} >
                            ...
                            <select name="testamento" value={selectedField.testamento} disabled={selectedField.versao == "selecione"} onChange={(evt) => { UpdateSelectedFields("testamento", evt.target.value, "testamento_id", evt.target.options.selectedIndex), ClickTestamento(evt.target.options.selectedIndex) }}>
                                <option value="selecione" disabled>Selecione</option>
                        ...
                        <div className={styles.modalopcoeslivro} >
                            ...
                            <select name="livro" value={selectedField.livro} disabled={selectedField.testamento == "selecione"} onChange={(evt) => { UpdateSelectedFields("livro", evt.target.value, "livro_id", evt.target.options.selectedIndex), ClickLivro(evt.target.options.selectedIndex) }}>
                                <option value="selecione" >Selecione</option>
                                {livros && livros.map((data: IresultLivros) => {
                                    return (<option key={data.livro_id} value={data.livro_id} >{data.livro_nome}</option>)
                                })}
                            </select>
                        </div>
                        <div className={styles.modalopcoescapitulo} >
                            <div className={styles.selectlabel} >Capítulo</div>
                            <select name="capitulo" value={selectedField.capitulo} disabled={selectedField.livro == "selecione"} onChange={(evt) => { UpdateSelectedFields("capitulo", evt.target.value, "capitulo_id", evt.target.options.selectedIndex) }}>
                                <option value="selecione" >Selecione</option>
                                {capitulo && capitulo}
                            </select>
                        </div>
